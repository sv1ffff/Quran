...
export default function SourateItem({ sourate }: Props) {
  ...
  return (
    ...
    >
      <Link
        to={`/${sourate.number}`}
        className="group relative block p-6 sm:p-8 bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-3xl hover:border-slate-300 dark:hover:border-slate-700 transition-all duration-300 shadow-sm"
      >
        <div className="flex items-center space-x-4 sm:space-x-6 relative z-10">
          <div
            className="w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white font-bold text-lg border border-slate-100 dark:border-slate-700"
          >
            {sourate.number}
          </div>
